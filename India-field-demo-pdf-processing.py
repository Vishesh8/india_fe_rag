...
pdf_loc = f"/Volumes/{catalog}/{schema}/{volume}"
...
# MAGIC %sql SELECT * FROM `$catalog`.`$schema`.`pdf_raw`
...
# MAGIC CREATE TABLE IF NOT EXISTS `$catalog`.`$schema`.`pdf_demo_tbl` (
...
(spark.readStream.table(f"`{catalog}`.`{schema}`.`pdf_raw`")
  ...
  .writeStream
    ...
    .table(f"`{catalog}`.`{schema}`.`pdf_demo_tbl`").awaitTermination())


# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * FROM  `$catalog`.`$schema`.`pdf_demo_tbl`
